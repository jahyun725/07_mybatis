...
/**
 * Mybatis Java Provider API를 이용한 동적 SQL 테스트 애플리케이션
 * SelectBuilder와 SqlBuilder의 동작을 확인하기 위한 메뉴 기반 프로그램
 */
public class Application {
  /**
   * 메인 메서드
   * SelectBuilder와 SqlBuilder 테스트 메뉴를 제공
   */
  ...
  /**
   * SelectBuilder 서브 메뉴
   * SelectBuilder를 이용한 기본 구문과 동적 SQL 테스트 메뉴 제공
   */
  private static void selectBuilderSubMenu() {
    ...
    do {
      ...
      switch (no) {
        case 1:
          selectBuilderService.testSimpleStatement();
          break;
        case 2:
          selectBuilderService.testDynamicStatement(inputSearchCriteria());
          break;
        case 9:
          return;
      }
  ...
  /**
   * 검색 조건 입력 메서드
   * @return SearchCriteria 검색 기준(name/category)과 검색어를 담은 객체
   */
  ...
  /**
   * SqlBuilder 서브 메뉴
   * SqlBuilder를 이용한 INSERT, UPDATE, DELETE 동적 SQL 테스트 메뉴 제공
   */
  private static void sqlBuilderSubMenu() {
    ...
    do {
      ...
      switch (no) {
        case 1:
          sqlBuilderService.registMenu(inputNewMenu());
          break;
        case 2:
          sqlBuilderService.modifyMenu(inputModifyMenu());
          break;
        case 3:
          sqlBuilderService.deleteMenu(inputMenuCode());
          break;
        case 9:
          return;
      }
  ...
  /**
   * 신규 메뉴 정보 입력 메서드
   * @return MenuDTO 등록할 메뉴 정보를 담은 객체
   */
  ...
  /**
   * 수정할 메뉴 정보 입력 메서드
   * @return MenuDTO 수정할 메뉴 정보를 담은 객체
   */
